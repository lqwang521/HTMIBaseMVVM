...
// 获取服务器响应状态码 key
NSString *const HTMI_BaseRequest_StatusCodeKey = @"statusCode";
// 服务器响应数据成功状态码 value
NSString *const HTMI_BaseRequest_DataValueKey = @"0000";
// 获取服务器响应状态信息 key
NSString *const HTMI_BaseRequest_StatusMsgKey = @"statusMsg";
// 获取服务器响应数据 key
NSString *const HTMI_BaseRequest_DataKey = @"data";;
...
- (NSError *)error
{
    ...
    // 获取服务器反馈 状态码 和 状态信息
    NSString *stCode = DecodeStringFromDic(responseDict, HTMI_BaseRequest_StatusCodeKey);
    NSString *stMsssage = DecodeStringFromDic(responseDict, HTMI_BaseRequest_StatusMsgKey);
...
- (BOOL)statusCodeValidator
{
    ...
    // 验证服务端返回验证码
    NSString *stCode = DecodeStringFromDic(responseDict, HTMI_BaseRequest_StatusCodeKey);
    if ([stCode isEqualToString:HTMI_BaseRequest_DataValueKey]) {
        // 请求成功
        return YES;
    }else{
        // 请求失败
